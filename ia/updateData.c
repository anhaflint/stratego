...
void updateBoard(const SGameState * const gameState, SMove enemyMovement)
{
	...
	on peut aisément conclure que c'est un scout. On vérifie si le mouvement est différent
	de (0,0) -> (0,0), qui serait un faux mouvement de l'ennemi (pénalité) */
	if ((enemyMovement.start.line != 0)
		&&(enemyMovement.start.col != 0)
		&&(enemyMovement.end.line != 0)
		&&(enemyMovement.end.col != 0)
		&&(((enemyMovement.start.line - enemyMovement.end.line)/2 != 0)
